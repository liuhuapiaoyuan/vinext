...
export default function Home() {
  ...
  return (
    <div>
      ...
      <p>
        Fetched From: <span id="fetched-from">{pngInfo?.fetchedFrom ?? "default"}</span>
      </p>
      <p>
        Content-Type: <span id="content-type">{pngInfo?.contentType ?? "default"}</span>
      </p>
      <p>
        Fetch Status: <span id="fetch-status">{pngInfo?.status ?? "default"}</span>
      </p>
    </div>
  );
}
