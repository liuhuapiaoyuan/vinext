...
test.describe("template navigation", () => {
  ...
  test("server template output stays stable from its index page to a child", async ({ page }) => {
    ...
    await expect(page.getByTestId("server-template-section-page")).toHaveText("Section alpha");
    await expect(page.getByTestId("server-template-render-id")).toHaveText(renderId!);
  });
});
