...
describe("createAppRscHandler", () => {
  ...
  it("snaps blur widths but rejects blur quality exceptions in production", async () => {
    const handler = createHandler({ isDev: false, imageConfig: { qualities: [75] } });
    const snappedWidth = await handler(
      new Request("https://example.test/docs/_next/image?url=%2Fimg.jpg&w=8&q=75"),
      null,
    );
    expect(snappedWidth.status).toBe(302);

    const blurQuality = await handler(
      new Request("https://example.test/docs/_next/image?url=%2Fimg.jpg&w=640&q=70"),
      null,
    );
    expect(blurQuality.status).toBe(400);
  });
  ...
  it("uses one isolated middleware branch for body-bearing RSC actions", async () => {
    ...
    expect(middleware).toHaveBeenCalledOnce();
    expect(handleServerActionRequest).toHaveBeenCalledOnce();
    // Middleware and the action each get a readable body. Fork clone helpers
    // may tee more than once for srvx/Bun safety, so do not pin clone count.
  });
