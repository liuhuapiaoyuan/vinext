...
function isImageShimModule(importer: string | undefined): boolean {
  if (!importer) return false;
  const normalized = toSlash(importer.split("?")[0]);
...
function isDefaultImageLoaderRequest(id: string, importer?: string): boolean {
  ...
  const normalizedId = toSlash(cleanId.split("?")[0]);
  const shimPath = toSlash(_defaultImageLoaderShimPath);
    ...
    },
    {
      ...
      async config(config, env) {
        ...
        if (options.appDir) {
          ...
          const hasRootPages = isDirectory(path.join(root, "pages"));
          const hasSrcApp = isDirectory(path.join(root, "src", "app"));
          const hasSrcPages = isDirectory(path.join(root, "src", "pages"));
